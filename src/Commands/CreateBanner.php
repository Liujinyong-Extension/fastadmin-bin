<?php
    ...
    class CreateBanner extends Command
    {
        ...
        protected function execute(InputInterface $input, OutputInterface $output)
        {
            ...
            exec($execC);
            exec($execM);
            $fileModel = 'application/admin/model/' . $controllerMenu . '.php';
            //复制修改后的文件到指定的位置 model
            if ($this->fs->exists($fileModel)) {
                $namespace = "app\\admin\\model;";
                if (strpos($controllerMenu, '/')) {
                    $namespace = "app\\admin\\model\\" . explode('/', $controllerMenu)[0] . ";";
                }
                $content = str_replace("{{namespace}}", $namespace, file_get_contents(__DIR__ . '/../stubs/banner/banner.model.stub'));
